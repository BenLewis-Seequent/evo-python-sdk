...
class FeedbackWidget(anywidget.AnyWidget, IFeedback):
...
class ObjectSearchWidget(anywidget.AnyWidget):
    """A widget for searching and selecting geoscience objects by name.

    This is a modern anywidget-based implementation.
    """

    _esm = _STATIC_DIR / "object_search.js"
    _css = _STATIC_DIR / "object_search.css"
    ...
    # Synced traits
    search_text = traitlets.Unicode("").tag(sync=True)
    object_type = traitlets.Unicode(allow_none=True, default_value=None).tag(sync=True)
    type_options = traitlets.List(traitlets.Tuple()).tag(sync=True)
    result_options = traitlets.List(traitlets.Tuple()).tag(sync=True)
    ...
    metadata_text = traitlets.Unicode("").tag(sync=True)
    ...
    def __init__(
        ...
        self._cached_objects: list[Any] = []
        self._cache_workspace_id: UUID | None = None
        self._selected_metadata: Any | None = None

        # Initialize type options
        self.type_options = [("All types", None)] + [
            (display_name, type_key) for type_key, display_name in self._TYPE_DISPLAY_NAMES.items()
        ]

        self.search_text = search
        self.object_type = object_type
        self.result_options = [("No results", None)]

        # Observe changes
        self.observe(self._on_search_change, names="search_text")
        self.observe(self._on_type_change, names="object_type")
        self.observe(self._on_selection_change, names="selected_id")
    ...
    async def _perform_search(self) -> None:
        ...
        try:
            ...
            if filtered:
                self.result_options = [
                    (f"{obj.name} ({self._get_type_display(obj.schema_id.sub_classification)})", str(obj.id))
                    for obj in filtered
                ]
                self.selected_id = str(filtered[0].id)
                self.status_text = f"Found {len(filtered)} object(s)"
            else:
                self.result_options = [("No matching objects", None)]
    ...
    async def _fetch_objects(self) -> None:
        ...
        if self.object_type:
            self._cached_objects = [
                obj for obj in all_objects if obj.schema_id.sub_classification == self.object_type
            ]
        else:
    ...
    async def _display_metadata(self, obj: Any) -> None:
        try:
            ...
            lines.append("=" * 60)
            lines.append(f"Type:        {self._get_type_display(obj.schema_id.sub_classification)}")
            lines.append(f"Path:        {obj.path}")
            lines.append(f"Object ID:   {obj.id}")
            lines.append(f"Schema:      {obj.schema_id}")
            lines.append("")
            lines.append(f"Created:     {obj.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
            lines.append(f"Modified:    {obj.modified_at.strftime('%Y-%m-%d %H:%M:%S')}")

            self.metadata_text = "\n".join(lines)
        except Exception as e:
            self.metadata_text = f"Error loading details: {e}"
    ...
    def selected_metadata(self) -> Any | None:
        """Get the full ObjectMetadata for the selected object."""
