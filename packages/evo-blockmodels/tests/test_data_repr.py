...
class TestVersionRepr(unittest.TestCase):
    ...
    def test_repr_html_returns_styled_table(self) -> None:
        """Test that _repr_html_ returns HTML with styled table."""
        version = self._create_test_version()
        html = version._repr_html_()

        # Should contain the stylesheet
        self.assertIn("<style>", html)
        self.assertIn('class="evo"', html)

        # Should contain title
        self.assertIn("Block Model Version", html)
        ...
        self.assertIn("Denis Simo", html)
        self.assertIn("Bounding Box", html)

        # Should contain columns table
    ...
    def test_repr_html_without_bbox(self) -> None:
        ...
        # Should show dash for missing bbox
        self.assertIn("Bounding Box", html)
        # The bbox value should be "-"
        self.assertIn('<td class="value">-</td>', html)

    def test_repr_html_without_comment(self) -> None:
        """Test that _repr_html_ works when comment is empty."""
        version = self._create_test_version(with_comment="")
        html = version._repr_html_()
